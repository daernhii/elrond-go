...
func (boot *baseBootstrap) ShouldSync() bool {
	...
	isNodeSynchronized := !boot.forkInfo.Detected && boot.hasLastBlock
...
func (boot *baseBootstrap) rollBack(revertUsingForkNonce bool) error {
	...
	for {
		...
		if revertUsingForkNonce && currHeader.GetNonce() > boot.forkInfo.Nonce {
			continue
		}
